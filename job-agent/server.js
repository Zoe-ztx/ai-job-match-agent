...
const backgroundAnalysisPrompt = require('./background-analysis-prompt');
...
// ===== 同背景去向分析专用系统提示词 =====
const BACKGROUND_ANALYSIS_SYSTEM_PROMPT = `你是"职引"AI同背景去向分析助手，专门帮助应届生了解与自己背景相似的求职者的就业去向情况。

你的核心能力：
1. 城市分布分析：分析同背景求职者去了哪些城市，各城市的人数/比例和平均薪资
2. 薪资水平分析：提供同背景求职者的薪资范围和分布
3. 岗位情况分析：分析同背景求职者去了哪些公司，担任什么岗位
4. 信息来源标注：每个数据点都要标注信息来源（链接或参考资料）

## 回复格式要求

### 当你进行同背景去向分析时，使用以下HTML格式：
用div class="background-report"包裹，包含：
- report-header: report-title + report-date(分析日期)
- report-summary: 一到两句话总体概括
- report-section: section-title + 内容
  - 常见章节：城市分布、薪资水平、岗位情况
- report-insight: 关键洞察或建议（用高亮框展示）

### 数据展示：
- 城市分布：用city-grid展示，每个city-card包含城市名、人数/比例、平均薪资、来源链接
- 薪资水平：用salary-ranges展示，每个salary-range包含薪资范围、人数/比例、来源链接
- 岗位情况：用job-table展示，每行包含岗位名称、公司、城市、薪资、来源链接
- 所有数据都要标注来源，使用 <a href="链接" target="_blank">来源名称</a> 格式

## 分析原则
1. 数据尽量基于搜索结果，如果搜索结果不足则基于你的知识并标注"预估值"
2. 分析要客观中立，给出具体数字和百分比
3. 建议要实用，给出具体的行动建议
4. 支持多轮对话，可以深入分析某个具体方面`;
...
app.post('/api/set-context', async (req, res) => {
  try {
    ...
    const funcLabels = { match: '岗位智能匹配', resume: '简历优化诊断', market: '同背景去向分析' };
    ...
    if (func === 'resume') {
      ...
      basePrompt = BACKGROUND_ANALYSIS_SYSTEM_PROMPT;
    }
...
app.post('/api/market-analysis', async (req, res) => {
  try {
    ...
    var analysisPrompt = backgroundAnalysisPrompt.replace('{userInfo}', userInfoStr).replace('{searchResults}', searchResultsText).replace(/{today}/g, today);

    // 4. 构建messages并调用LLM
    var messages = [
      { role: 'system', content: '你是"职引"AI同背景去向分析助手，专门帮助应届生了解与自己背景相似的求职者的就业去向情况。你的分析要客观、数据驱动、有可操作性，重点分析城市分布、薪资水平和岗位情况，并标注信息来源。输出纯HTML，不要用代码块包裹。' },
    ...
    if (session.length === 0 || session[0].role !== 'system') {
      session.length = 0;
      session.push({ role: 'system', content: '你是"职引"AI同背景去向分析助手，专门帮助应届生了解与自己背景相似的求职者的就业去向情况。你可以：1）分析城市分布和薪资水平 2）解读岗位情况和公司去向 3）给出求职策略建议。输出纯HTML时不要用代码块包裹。' });
    }
    session.push({ role: 'user', content: '帮我分析同背景求职者的去向情况' });
    session.push({ role: 'assistant', content: reply });
  ...
  try {
    ...
    if (session.length === 0) {
      ...
      if (mode === 'resume') {
        ...
        chatPrompt = BACKGROUND_ANALYSIS_SYSTEM_PROMPT;
      }
