...
// ===== 简历优化专用系统提示词 =====
const RESUME_SYSTEM_PROMPT = `你是"职引"AI简历优化助手，专门帮助应届生诊断和优化简历。

你的核心能力：
1. 简历诊断：分析简历的格式、内容、关键词等问题，给出评分
2. 优化建议：针对具体问题给出可操作的改进建议
3. 改写示例：用STAR法则（情境-任务-行动-结果）改写经历描述
4. 关键词优化：针对目标岗位调整简历关键词，提高ATS通过率

## 回复格式要求

### 当你诊断简历时，使用以下HTML格式：
用div class="diagnose-report"包裹，包含：
- diagnose-header: diagnose-title + diagnose-score-wrap(分数/100)
- diagnose-score-bar + diagnose-score-fill(宽度=分数%)
- diagnose-section: section-title"综合评价" + diagnose-summary
- diagnose-section: section-title"发现的问题" + issue-card(issue-error红色/issue-warn黄色)
- diagnose-section: section-title"优化建议" + suggest-card(带编号)
- diagnose-section: section-title"改写示例（STAR法则）" + rewrite-card(rewrite-before + rewrite-after)
...
## 重要规则
1. 评分要客观，不要一味夸奖，指出真实问题
2. 改写示例要保留原意但提升表达力和量化程度
3. 保持专业但亲切的语气，像学长/学姐给建议
4. 支持多轮对话，记住之前讨论的内容
5. 如果用户要求改写某段经历，优先用STAR法则
6. 不要预设用户的任何背景信息，一切从对话中获取`;

// ===== 市场竞争分析专用系统提示词 =====
const MARKET_SYSTEM_PROMPT = `你是"职引"AI市场分析助手，专门帮助应届生了解就业市场的竞争情况。

你的核心能力：
1. 市场供需分析：分析目标岗位/行业在指定城市的供需情况
2. 薪资水平分析：提供行业薪资范围和趋势
3. 技能需求分析：分析热门技能和用户技能匹配度
4. 行业趋势分析：判断行业发展阶段和未来趋势
5. 同背景去向：分析类似背景求职者的就业去向

## 回复格式要求

### 当你进行市场分析时，使用以下HTML格式：
用div class="market-report"包裹，包含：
- market-header: market-title + market-date(分析日期)
- market-summary: 一到两句话总体概括市场情况
- market-grid: 2x2网格布局，包含4个market-card
  - market-card: card-icon + card-title + card-value + card-desc
  - 卡片内容：供需比、平均薪资、热门技能数量、行业趋势
- market-section: section-title + section-content
  - 常见章节：岗位供需详情、薪资分布、技能要求、同背景去向、行业趋势与建议
- market-insight: 关键洞察或建议（用高亮框展示）

### 图表和数据展示：
- 使用HTML/CSS模拟简单图表（如进度条、彩色标签等）
- 数据要具体，尽量给出数字和百分比
- 如果数据不确定，给出合理估算并标注"预估值"

## 重要规则
1. 分析要客观中立，不要夸大或贬低市场情况
2. 数据尽量基于最新信息（联网搜索结果）
3. 如果用户背景信息不足，可以给出一般性分析，但要说明局限性
4. 保持专业但易懂的语气，避免过多专业术语
5. 支持多轮对话，可以深入分析某个具体方面
6. 建议要实用，给出具体的行动建议（如：应该重点提升哪些技能）`;
...
app.post('/api/set-context', async (req, res) => {
  try {
    ...
    if (func === 'resume') {
      basePrompt = RESUME_SYSTEM_PROMPT;
    } else if (func === 'market') {
      basePrompt = MARKET_SYSTEM_PROMPT;
    }
...
// ===== 简历诊断接口 =====
app.post('/api/diagnose-resume', async (req, res) => {
  try {
    const { userInfo, sessionId } = req.body;
    if (!userInfo) {
      return res.status(400).json({ error: '请先提供简历信息' });
    }

    // 1. 构建用户信息摘要
    const labels = { edu: '学历背景', skills: '技术技能', projects: '项目经历', intern: '实习/工作经历', salary: '期望薪资', city: '意向城市', direction: '求职方向' };
    const userInfoStr = Object.entries(userInfo)
      .filter(([k, v]) => v && v !== '未识别' && v !== '未填写' && v !== '未识别，请填写')
      .map(([k, v]) => labels[k] + '：' + v)
      .join('\n');
    ...
    // 2. 构建诊断提示词
    var today = new Date().toISOString().split('T')[0];
    var diagnosePrompt = '以下是用户的简历信息：\n\n' + userInfoStr + '\n\n请对这份简历进行全面诊断，按以下格式输出纯HTML（不要用代码块包裹）：\n\n<div class="diagnose-report">\n  <div class="diagnose-header">\n    <div class="diagnose-title">简历诊断报告</div>\n    <div class="diagnose-score-wrap">\n      <span class="diagnose-score">75</span><span class="diagnose-score-unit">/100</span>\n    </div>\n  </div>\n  <div class="diagnose-score-bar"><div class="diagnose-score-fill" style="width:75%"></div></div>\n  \n  <div class="diagnose-section">\n    <div class="section-title">综合评价</div>\n    <div class="diagnose-summary">一两句话总体评价简历质量</div>\n  </div>\n  \n  <div class="diagnose-section">\n    <div class="section-title">发现的问题</div>\n    <div class="issue-card issue-error">\n      <div class="issue-icon">&#10060;</div>\n      <div class="issue-content">\n        <div class="issue-title">问题标题</div>\n        <div class="issue-desc">问题描述</div>\n      </div>\n    </div>\n    <div class="issue-card issue-warn">\n      <div class="issue-icon">&#9888;&#65039;</div>\n      <div class="issue-content">\n        <div class="issue-title">问题标题</div>\n        <div class="issue-desc">问题描述</div>\n      </div>\n    </div>\n  </div>\n  \n  <div class="diagnose-section">\n    <div class="section-title">优化建议</div>\n    <div class="suggest-card">\n      <div class="suggest-num">1</div>\n      <div class="suggest-content">\n        <div class="suggest-title">建议标题</div>\n        <div class="suggest-desc">具体建议内容</div>\n      </div>\n    </div>\n  </div>\n  \n  <div class="diagnose-section">\n    <div class="section-title">改写示例（STAR法则）</div>\n    <div class="rewrite-card">\n      <div class="rewrite-label rewrite-before">原文</div>\n      <div class="rewrite-text">用户原始描述（如有）</div>\n      <div class="rewrite-label rewrite-after">改写后</div>\n      <div class="rewrite-text">用STAR法则改写后的版本</div>\n    </div>\n  </div>\n</div>\n\n【评分规则】满分100，扣分项：信息不完整(-10~20)、描述太笼统(-5~15)、缺少量化成果(-10)、技能与岗位不匹配(-5~10)、格式不规范(-5)\n【问题分类】严重问题用issue-error(红色)，一般问题用issue-warn(黄色)，列出3-5个问题\n【建议数量】3-4条优化建议\n【改写示例】至少1个改写示例，用STAR法则改写经历描述\n今天是' + today + '。直接输出纯HTML，不要用代码块包裹。';

    // 3. 构建messages
    var messages = [
      { role: 'system', content: '你是"职引"AI简历优化助手，专门帮助应届生诊断和优化简历。你的诊断要专业、具体、有可操作性。输出纯HTML，不要用代码块包裹。' },
      { role: 'user', content: diagnosePrompt }
    ];

    console.log('Calling LLM for resume diagnosis, prompt length:', diagnosePrompt.length);
    ...
    if (session.length === 0 || session[0].role !== 'system') {
      session.length = 0;
      session.push({ role: 'system', content: '你是"职引"AI简历优化助手，专门帮助应届生诊断和优化简历。你可以：1）给出简历优化建议 2）用STAR法则改写经历描述 3）针对目标岗位优化关键词。输出纯HTML时不要用代码块包裹。' });
    }
    session.push({ role: 'user', content: '帮我诊断简历' });
    session.push({ role: 'assistant', content: reply });

    res.json({ reply: reply });
  } catch (err) {
    console.error('Diagnose resume error:', err.message);
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/api/market-analysis', async (req, res) => {
  try {
    ...
    const searchKeywords = [
      `${direction} ${city} 招聘 市场 2025`,
      `${direction} ${city} 薪资 就业 竞争`
    ];

    console.log('Searching market data for:', searchKeywords);

    let allResults = [];
    for (const keyword of searchKeywords.slice(0, 2)) {  // 最多搜2组关键词
      try {
        const results = await searchJobs(keyword);
        allResults = allResults.concat(results);
      } catch (e) {
        console.error('Search error for keyword "' + keyword + '":', e.message);
      }
    }
    ...
    // 3. 构建分析提示词
    var today = new Date().toISOString().split('T')[0];
    var searchResultsText = uniqueResults.length > 0
      ? '以下是联网搜索到的市场数据（' + uniqueResults.length + '条）：\n' + uniqueResults.map((r, i) => {
          return `[搜索结果${i+1}] ${r.title}\n链接：${r.link}\n摘要：${(r.snippet || '').substring(0, 200)}`;
        }).join('\n\n')
      : '（未搜索到相关数据，请基于你的知识进行分析）';

    var analysisPrompt = '以下是用户的背景信息：\n\n' + userInfoStr + '\n\n' + searchResultsText + '\n\n请对这位用户的就业市场情况进行全面分析，按以下格式输出纯HTML（不要用代码块包裹）：\n\n<div class="market-report">\n  <div class="market-header">\n    <div class="market-title">市场竞争分析报告</div>\n    <div class="market-date">分析日期：' + today + '</div>\n  </div>\n  <div class="market-summary">一到两句话总体概括市场情况</div>\n  \n  <div class="market-grid">\n    <div class="market-card">\n      <div class="card-icon">📊</div>\n      <div class="card-title">岗位供需比</div>\n      <div class="card-value">数值</div>\n      <div class="card-desc">简短说明</div>\n    </div>\n    <div class="market-card">\n      <div class="card-icon">💰</div>\n      <div class="card-title">平均薪资</div>\n      <div class="card-value">范围</div>\n      <div class="card-desc">简短说明</div>\n    </div>\n    <div class="market-card">\n      <div class="card-icon">🔧</div>\n      <div class="card-title">热门技能</div>\n      <div class="card-value">数量</div>\n      <div class="card-desc">简短说明</div>\n    </div>\n    <div class="market-card">\n      <div class="card-icon">📈</div>\n      <div class="card-title">行业趋势</div>\n      <div class="card-value">上升/平稳/下降</div>\n      <div class="card-desc">简短说明</div>\n    </div>\n  </div>\n  \n  <div class="market-section">\n    <div class="section-title">岗位供需详情</div>\n    <div class="section-content">详细分析内容...</div>\n  </div>\n  \n  <div class="market-section">\n    <div class="section-title">薪资分布</div>\n    <div class="section-content">薪资范围和分布...</div>\n  </div>\n  \n  <div class="market-section">\n    <div class="section-title">技能要求</div>\n    <div class="section-content">热门技能和匹配度...</div>\n  </div>\n  \n  <div class="market-section">\n    <div class="section-title">同背景求职者去向</div>\n    <div class="section-content">类似背景的人去了哪些公司...</div>\n  </div>\n  \n  <div class="market-section">\n    <div class="section-title">行业趋势与建议</div>\n    <div class="section-content">未来趋势和行动建议...</div>\n  </div>\n  \n  <div class="market-insight">\n    <div class="insight-title">🔍 关键洞察</div>\n    <div class="insight-content">最重要的1-2个发现</div>\n  </div>\n</div>\n\n【分析要求】\n1. 数据尽量基于搜索结果，如果搜索结果不足则基于你的知识并标注"预估值"\n2. 分析要客观中立，给出具体数字和百分比\n3. 同背景去向部分要具体（公司名、岗位名、薪资等）\n4. 建议要实用，给出具体的行动建议\n今天是' + today + '。直接输出纯HTML，不要用代码块包裹。';

    // 4. 构建messages并调用LLM
    var messages = [
      { role: 'system', content: '你是"职引"AI市场分析助手，专门帮助应届生了解就业市场的竞争情况。你的分析要客观、数据驱动、有可操作性。输出纯HTML，不要用代码块包裹。' },
      { role: 'user', content: analysisPrompt }
    ];

    console.log('Calling LLM for market analysis, prompt length:', analysisPrompt.length);
    ...
    if (session.length === 0 || session[0].role !== 'system') {
      session.length = 0;
      session.push({ role: 'system', content: '你是"职引"AI市场分析助手，专门帮助应届生了解就业市场的竞争情况。你可以：1）分析市场供需和薪资 2）解读行业趋势 3）给出求职策略建议。输出纯HTML时不要用代码块包裹。' });
    }
    session.push({ role: 'user', content: '帮我分析市场竞争情况' });
    session.push({ role: 'assistant', content: reply });
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    // 如果是新会话，根据mode添加对应系统提示词
    if (session.length === 0) {
      var chatPrompt = SYSTEM_PROMPT;
      if (mode === 'resume') {
        chatPrompt = RESUME_SYSTEM_PROMPT;
      } else if (mode === 'market') {
        chatPrompt = MARKET_SYSTEM_PROMPT;
      }
      session.push({ role: 'system', content: chatPrompt });
    }
